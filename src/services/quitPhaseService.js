...
export const getDefaultPhases = async (addictionLevel) => {
  try {
    ...
    console.log(
      "Using mock data for default phases due to unexpected response format"
    );
...
export const createGoalsOfPhases = async (quitPlanId, phasesData) => {
  try {
    ...
    const response = await axiosInstance.post(
      "/api/quit-phases/default/create-goals",
      phasesData,
      {
        params: { quitPlanId },
      }
    ...
    if (
      ...
      error.response.data.includes("quitPlanId not found")
    ) {
