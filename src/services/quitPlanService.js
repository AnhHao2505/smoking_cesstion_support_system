...
export const getDefaultQuitPhases = () => {
  return [
    ...
    },
    {
      ...
      phase_order: 4,
      objective:
        "Hoàn thành quá trình cai thuốc, xây dựng lối sống không thuốc lá",
      is_completed: false,
    },
  ];
};
...
// Get newest quit plan of member - Updated to handle API limitations
export const getNewestQuitPlan = async (memberId = null) => {
  try {
    if (memberId) {
      console.warn(
        "API /api/quit-plans/newest does not support memberId parameter"
      );
      console.log("Returning null to prevent server errors");

      // Since the API doesn't support getting plans for other members,
      // we'll return null to indicate no data available
      // This prevents the 500 server error from happening
      return null;
    } else {
      // Member getting their own newest plan
      const response = await axiosInstance.get("/api/quit-plans/newest");
      console.log("getNewestQuitPlan API response:", response);

      // Handle the response properly
      const processedResponse = handleApiResponse(response);
      console.log("getNewestQuitPlan processed response:", processedResponse);

      return processedResponse.success ? processedResponse.data : null;
    }
  } catch (error) {
...
export const getQuitPlanDetail = async (planId) => {
  try {
    ...
    console.warn(
      "No direct endpoint for plan by ID, this would need to be implemented based on business logic"
    );
    return {
      success: true,
...
const getQuitPlanDetailMock = (planId) => {
  return {
    ...
    end_date: "2025-07-01",
    strategies_to_use:
      "Nicotine replacement therapy, daily exercise, mindfulness meditation",
    medications_to_use: "Nicotine patches, gum as needed",
    medication_instructions:
      "Apply patch every morning. Use gum when experiencing strong cravings (max 8 pieces per day).",
    preparation_steps:
      "Remove all cigarettes and smoking accessories from home and workplace. Inform friends and family about quit date and ask for support. Stock up on healthy snacks and water.",
    ...
    quit_phases: [
      ...
      },
      {
        ...
        is_completed: false,
        objective:
          "Implement strategies to manage cravings and maintain abstinence",
      },
      {
        quit_phase_id: 4,
...
export const formatQuitPlanForMember = (planData) => {
  ...
  return {
    ...
    medication_instructions:
      planData.medicationInstructions || planData.medication_instructions,
    preparation_steps:
      planData.relapsePreventionStrategies || planData.preparation_steps,
    ...
    statusText: getQuitPlanStatusText(
      planData.quitPlanStatus || planData.status
    ),
    statusColor: getQuitPlanStatusColor(
      planData.quitPlanStatus || planData.status
    ),
  };
};
...
export const getMemberQuitPlanWithActions = async (memberId) => {
  try {
    ...
    // For members getting their own plan, don't pass memberId
    // because the API endpoint only works for authenticated user
    const response = await getNewestQuitPlan(); // Remove memberId parameter

    console.log("getMemberQuitPlanWithActions response:", response);

    // Handle null response gracefully
    if (!response || response === null) {
      console.log("No plan data available for member");
      return null;
    }

    // If response is already processed, return it directly
    if (response.success !== undefined) {
      return response.success ? response.data : null;
    }

    // Otherwise process the response
    const processedResponse = handleApiResponse({
      data: response,
      status: 200,
    });
    return processedResponse.success ? processedResponse.data : null;
 ...
 */
export const processMemberQuitPlanAction = async (
  planId,
  action,
  options = {}
) => {
  try {
    console.log(
      `Processing member action: ${action} on plan:`,
      planId,
      "Options:",
      options
    );
 ...
 */
export const createQuitPlanForMember = async (
  memberId,
  planData,
  phaseData = null
) => {
  try {
    console.log(
      "Coach creating quit plan for member:",
      memberId,
      "Plan data:",
      planData
    );
...
export const formatQuitPlanForCoach = (planData) => {
  ...
  return {
    ...
    canEdit:
      formatted.status === "PENDING_APPROVAL" || formatted.status === "ACTIVE",
    canFinish: formatted.status === "ACTIVE",
    canDisable:
      formatted.status === "ACTIVE" || formatted.status === "PENDING_APPROVAL",

    // Coach action helpers
    isWaitingForMemberResponse:
      formatted.status === "PENDING_APPROVAL" || formatted.status === "CREATED",
  ...
  const notifications = {
    coach: {
      ...
      ACTIVE: {
        type: "success",
        message:
          "Thành viên đã chấp nhận kế hoạch và bắt đầu hành trình cai thuốc",
      },
      DENIED: {
        type: "warning",
        message:
          "Thành viên đã từ chối kế hoạch này. Vui lòng tạo kế hoạch mới.",
      },
      COMPLETED: {
        type: "success",
    ...
    member: {
      PENDING_APPROVAL: {
        type: "info",
        message:
          "Huấn luyện viên đã tạo kế hoạch mới cho bạn. Vui lòng xem xét và phê duyệt.",
      },
      ACTIVE: {
        type: "success",
      ...
      DENIED: {
        type: "info",
        message:
          "Bạn đã từ chối kế hoạch này. Huấn luyện viên sẽ tạo kế hoạch mới.",
      },
      COMPLETED: {
        type: "success",
...
export const validateQuitPlanData = (planData) => {
  ...
  if (planData.startDate && planData.endDate) {
    ...
    if (durationDays < 7) {
      warnings.push(
        "Kế hoạch quá ngắn (dưới 1 tuần). Khuyến nghị ít nhất 4-12 tuần."
      );
...
export const generateQuitPlanSummary = (quitPlan) => {
  ...
  return {
    ...
    statusText: getQuitPlanStatusText(
      quitPlan.status || quitPlan.quitPlanStatus
    ),
    statusColor: getQuitPlanStatusColor(
      quitPlan.status || quitPlan.quitPlanStatus
    ),
    duration: {
      ...
      text:
        durationDays <= 7
          ? `${durationDays} ngày`
          : `${Math.ceil(durationDays / 7)} tuần`,
    },
    startDate: startDate.toLocaleDateString("vi-VN"),
    endDate: endDate.toLocaleDateString("vi-VN"),
    hasStrategies: !!(quitPlan.copingStrategies || quitPlan.strategies_to_use),
    hasMedications: !!(
      quitPlan.medicationsToUse || quitPlan.medications_to_use
    ),
    hasPreparationSteps: !!(
      quitPlan.relapsePreventionStrategies || quitPlan.preparation_steps
    ),
  };
};
