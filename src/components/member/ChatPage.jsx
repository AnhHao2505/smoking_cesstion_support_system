...
const ChatPage = () => {
  ...
  const handleRoomSelect = async (room) => {
    try {
      ...
      setSelectedChatRoom(room); // <- ✅ đặt log ngay dưới dòng này
      console.log("Selected chat room:", room); // 👉 thêm ở đây để debug
  ...
  const handleVideoCall = () => {
    ...
    // Check if either selectedCoach or selectedChatRoom exists
    if (!selectedCoach && !selectedChatRoom) {
      message.warning(
        "Vui lòng chọn huấn luyện viên hoặc phòng chat để gọi video"
      );
      return;
    }
  ...
  return (
    <Layout style={{ height: "100vh", background: "#f0f2f5" }}>
                    ...
                    description={
                      <div>
                        <Text type="secondary" style={{ fontSize: "12px" }}>
                          Phòng{" "}
                          {room.type === "PRIVATE" ? "riêng tư" : "cộng đồng"}
                        </Text>
                        <br />
              ...
              >
                <Space>
                  ...
                  <div>
                    ...
                    <Text type="secondary" style={{ fontSize: "12px" }}>
                      Mã phòng: {selectedChatRoom.roomId} •{" "}
      ...
      {/* Video Call Modal */}
      {isVideoCallVisible && (selectedCoach || selectedChatRoom) && (
        <VideoCallModal
          appointmentId={selectedChatRoom?.appointmentId || 1} // Default appointmentId nếu không có
          isConsultant={
            currentUser?.role === "COACH" || currentUser?.userRole === "COACH"
          }
          onCallEnd={handleCloseVideoCall}
        />
      )}
    </Layout>
  );
