...
const DailyCheckIn = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (statusData) {
          ...
          // Don't show message here, let user try to submit and get proper error message
        }
  ...
  const handleSubmit = async () => {
    ...
    if (hasSubmittedAssessment) {
      message.warning(
        "Bạn đã thực hiện đánh giá này rồi. Mỗi thành viên chỉ được đánh giá một lần duy nhất."
      );
      return;
    }
    ...
    try {
      ...
      let response;
      // Always use create endpoint since backend doesn't have update endpoint
      ...
      // Check if it's a rate limit error (max = 1)
      if (
        error.message &&
        ...
        message.error(
          error.message ||
            "Có lỗi xảy ra khi cập nhật đánh giá. Vui lòng thử lại sau."
        );
      }
    } finally {
      setSubmitting(false);
          ...
          <Row gutter={[16, 16]} className="mb-4">
            ...
            <Col xs={24} md={8}>
              <Card size="small" className="stat-mini">
                ...
                <br />
                <Tag color="blue">
                  {quitPlan?.current_phase?.phase_name || "Chuẩn bị"}
                </Tag>
              </Card>
            </Col>
            <Col xs={24} md={8}>
