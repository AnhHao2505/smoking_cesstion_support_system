...
const MemberQuitPlanFlow = () => {
  ...
  const submitPlanAction = async () => {
    try {
      ...
      if (actionType === 'accept') {
        ...
        if (dateError) {
          message.error(dateError || 'Ngày bắt đầu không hợp lệ. Vui lòng kiểm tra lại.');
          return;
        }
      ...
      if (actionType === 'accept') {
        ...
        console.log(`Accepting plan with ID ${planId} and start date ${validStartDate}`);
      ...
      if (response.success) {
        ...
        if (actionType === 'accept') {
          ...
          const calculatedEndDate = selectedDateObj.clone().add(currentPlan.durationInDays, 'days').format('YYYY-MM-DD');
        ...
        const actionMessage = actionType === 'accept'
          ? `Kế hoạch đã được chấp nhận! Hành trình cai thuốc của bạn sẽ bắt đầu từ ${selectedDateObj.format('DD/MM/YYYY')}.`
  ...
  const calculateProgress = () => {
    ...
    const isValidProgress = currentPlan.progressInDay !== undefined && currentPlan.progressInDay !== null && !isNaN(currentPlan.progressInDay);
    const isValidDuration = currentPlan.durationInDays !== undefined && currentPlan.durationInDays !== null && !isNaN(currentPlan.durationInDays);
    ...
    <div className="member-quit-plan-flow">
      <div className="container py-4">
        ...
        {!currentPlan ? (
          ...
          <Card>
            <div className="text-center py-4">
              ...
              <Title level={3} style={{ marginBottom: '20px', color: '#1890ff' }}>Chưa có kế hoạch cai thuốc</Title>
              <Paragraph style={{ fontSize: '16px', marginBottom: '20px' }}>
                Bạn chưa có kế hoạch cai thuốc nào. Hãy bắt đầu hành trình cai thuốc của mình bằng cách yêu cầu kế hoạch từ huấn luyện viên.
              </Paragraph>
              
              <Alert 
                type="warning" 
                showIcon
                style={{ marginBottom: '20px', textAlign: 'left', maxWidth: '800px', margin: '0 auto 20px' }} 
                message="Tính năng dành cho thành viên Premium" 
                description={
                  <div>
                    <Paragraph style={{ marginBottom: '8px' }}>
                      Kế hoạch cai thuốc là tính năng dành riêng cho thành viên Premium. 
                      Hãy nâng cấp tài khoản của bạn thông qua nút <strong>"Nâng cấp Premium"</strong> trên thanh điều hướng để được tiếp cận với huấn luyện viên chuyên nghiệp và kế hoạch cai thuốc cá nhân hóa.
                    </Paragraph>
                  </div>
                }
              />
              ...
              <Alert
                ...
                description={
                  <div>
                    <Paragraph style={{ marginBottom: '8px' }}>
                      Để có kế hoạch cai thuốc phù hợp nhất với tình trạng của bạn, chúng tôi khuyến nghị:
                    </Paragraph>
                    ...
                    <Paragraph>
                      Sau khi gửi yêu cầu, huấn luyện viên sẽ tạo kế hoạch cai thuốc cá nhân hóa cho bạn
                       dựa trên thông tin sức khỏe và mục tiêu của bạn khi tư vấn.
                    </Paragraph>
                  </div>
                }
                type="info"
                showIcon
                style={{ maxWidth: '700px', margin: '0 auto 20px', textAlign: 'left' }}
              />
                ...
                <Button 
                  ...
                  onClick={() => {
                    Modal.confirm({
                      ...
                      content: 'Khi gửi yêu cầu, huấn luyện viên sẽ nhận thông báo và tạo kế hoạch cai thuốc cá nhân hóa cho bạn. Bạn sẽ nhận được thông báo khi kế hoạch được tạo.',
                      okText: 'Gửi yêu cầu',
                      cancelText: 'Hủy',
                      onOk: async () => {
                        try {
                          ...
                          if (response.success) {
                            message.success(response.message || 'Đã gửi yêu cầu thành công!');
                            // Cập nhật lại dữ liệu sau khi yêu cầu thành công
                            setTimeout(() => fetchMemberQuitPlanData(), 1000);
                          } else {
                            message.error(response.message || 'Không thể gửi yêu cầu');
                          }
                        } catch (error) {
                          message.error('Có lỗi xảy ra: ' + (error.message || 'Không thể gửi yêu cầu'));
                        }
                      }
                    });
                  }}
                  style={{ width: '100%', marginTop: '20px', height: '50px', fontSize: '16px' }}
                >
            ...
            <Col xs={24}>
                ...
                title={
                  <Space>
                    ...
                    <Tag 
                      ...
                      icon={
                        currentPlan.status === 'IN_PROGRESS' ? <ClockCircleOutlined /> : 
                        currentPlan.status === 'COMPLETED' ? <CheckCircleOutlined /> :
                        currentPlan.status === 'FAILED' ? <CloseCircleOutlined /> :
                        currentPlan.status === 'PENDING' ? <ClockCircleOutlined /> :
                        currentPlan.status === 'REJECTED' ? <CloseCircleOutlined /> :
                        currentPlan.status === 'ACCEPTED' ? <CheckCircleOutlined /> : null
                      }
                    >
              ...
              >
                  ...
                  <Alert
                    ...
                    description={
                      <div>
                        ...
                        <p>Thời gian thực hiện: <strong>{currentPlan.durationInDays} ngày</strong> (Ngày bắt đầu (trong vòng 6 tháng kể từ hôm nay) sẽ được xác định khi bạn chấp nhận kế hoạch)</p>
                ...
                <Row gutter={[16, 16]}>
                  <Col xs={24} md={12}>
                    <Descriptions column={1} size="small">
                      ...
                      <Descriptions.Item label="Thời gian thực hiện">
                        {currentPlan.durationInDays || moment(currentPlan.end_date).diff(moment(currentPlan.start_date), 'days')} ngày
                      </Descriptions.Item>
                  ...
                  <Col xs={24} md={12}>
                    {(currentPlan.status === 'IN_PROGRESS' || currentPlan.status === 'COMPLETED' || currentPlan.status === 'FAILED') && (
                      <div>
                              ...
                              <div className="mt-3">
                                ...
                                <div style={{ 
                                  ...
                                  {currentPlan.status === 'COMPLETED' && (
                                    <Tag 
                                      color={currentPlan.completionQuality?.toLowerCase().includes('cải thiện') ? 'orange' : 'green'}
                                      style={{ 
                                        fontSize: '11px',
                                        fontWeight: 'bold',
                                        padding: '2px 6px',
                                      ...
                                      icon={<CheckCircleOutlined />}
                                    >
                                      {currentPlan.completionQuality || progress.quality || 'Hoàn thành'}
                                    </Tag>
                                  )}
                    ...
                    {currentPlan.status === 'COMPLETED' ? (
                      <>
                        ...
                        {currentPlan.finalEvaluation ? (
                          ...
                          <Paragraph>
                            Đang chờ huấn luyện viên {currentPlan.coach_name} đánh giá cuối cùng về kế hoạch cai thuốc của bạn.
                          </Paragraph>
                        )}
                      </>
                    ) : (
                      <>
                        ...
                        <Paragraph>
                          Cai thuốc là một hành trình khó khăn. Đừng nản lòng, hãy rút kinh nghiệm và thử lại với kế hoạch mới.
                        </Paragraph>
                      </>
                    )}
                    <Divider />
                    <Button 
                      ...
                      onClick={() => {
                        Modal.confirm({
                          ...
                          content: 'Khi gửi yêu cầu, huấn luyện viên sẽ nhận thông báo và tạo kế hoạch cai thuốc cá nhân hóa mới cho bạn.',
                          okText: 'Gửi yêu cầu',
                          cancelText: 'Hủy',
                          onOk: async () => {
                            try {
                              const response = await requestQuitPlan();
                              if (response.success) {
                                message.success(response.message || 'Đã gửi yêu cầu kế hoạch mới thành công!');
                              } else {
                                message.error(response.message || 'Không thể gửi yêu cầu');
                              }
                            } catch (error) {
        ...
        <Modal
          ...
          okText={actionType === 'accept' ? 'Chấp nhận kế hoạch' : 'Từ chối kế hoạch'}
          okButtonProps={{
        ...
        >
          ...
          {actionType === 'accept' && (
            <div>
              <Form.Item
                label="Ngày bắt đầu"
                help={dateError || "Chọn ngày bạn muốn bắt đầu kế hoạch cai thuốc"}
                ...
                <DatePicker
                  ...
                  onChange={(date) => {
                    ...
                    if (date.isBefore(today)) {
                      setDateError('Ngày bắt đầu không thể là ngày trong quá khứ');
                      return;
                    }
                    
                    const sixMonthsLater = moment().add(6, 'months');
                    if (date.isAfter(sixMonthsLater)) {
                      setDateError('Ngày bắt đầu không thể xa hơn 6 tháng từ hôm nay');
                      return;
                    }
